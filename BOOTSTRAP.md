...
- [ ] Run `gh pr checks 1` and **copy the exact status-check names that appear** — context names vary per bot installation + version. Don't hardcode names you expect; use whatever `gh pr checks` shows.
...
- [ ] Branch rules — CHECK:
  ...
  - ✅ Require status checks to pass
    ...
    - Add the **observed** context names from step 4's `gh pr checks` output (not hardcoded guesses), + any CI workflow names (e.g., `file-size-gate`, `quality`, `lint`)
  - ✅ Block force pushes
- [ ] LEAVE UNCHECKED:
...
```
~/.claude/projects/<home-derived-slug>/memory/feedback_two_bot_gate_active.md
```

The `<home-derived-slug>` is generated by `bootstrap.sh` from `$HOME/Developer/<owner>/<repo>` (per-machine path with `/` replaced by `-`). Run `./bootstrap.sh <owner>/<repo>` first; it creates the slug directory and shows the path. Example for a user with home `/home/jane` and repo `acme/widget`: slug is `home-jane-Developer-acme-widget`.
